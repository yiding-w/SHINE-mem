...
# -*- coding: utf-8 -*-
"""
MS MARCO Multi-QA Conversation-as-Context Dataset Module for Pretrain Annealing
(Preserve Thinking Mode)

This module provides a pretrain annealing dataset based on the MS MARCO Multi-QA
dataset, using the preserve_thinking mode (default chat template with
enable_thinking=True and preserve_thinking=True).

Unlike the SFT version where context is the evidence passage, here the
context is a COPY of the conversation tokens. This enforces that context_seq_length
must equal conv_seq_length.

Data format (JSONL):
    Each line is a JSON object with:
        - query_id: int
        - context: str (the evidence passage — NOT used as context here)
        - conversations: list of {question: str, answer: str}

Preprocessing:
    Tokenizes all samples using the tokenizer's default chat template
    (with preserve_thinking=True) for the conversation part, saves as .npz
    shard files. Context tokens are NOT stored separately since context
    is a copy of conversation at collation time.

Dataset:
    On construction, loads pre-tokenized shards and builds an index for
    random access. Each sample returns pre-tokenized conversation token ids
    with labels.

Collator:
    Pads/truncates to fixed lengths and produces the unified batch format:
        - context_ids:      (B, context_total_len)  where context_total_len = conv_seq_length + num_mem_token
        - conversation_ids: (B, conv_seq_length)
        - labels:           (B, conv_seq_length)  with -100 for non-content tokens
        - context_lengths:  (B,)  actual context lengths (= actual conv lengths, before padding)

    Context is a COPY of conversation_ids (without mem token positions).
"""
...
def _load_train_data(data_cfg) -> List[Dict]:
    ...
    for fname in train_files:
        ...
        logger.info(f"[msmarco_mqa_conv_preserve_thinking] Loaded {len(items)} samples from {fname}")
        all_items.extend(items)

    logger.info(f"[msmarco_mqa_conv_preserve_thinking] Total training samples: {len(all_items)}")
    return all_items
...
def _load_val_data(data_cfg) -> List[Dict]:
    ...
    items = _load_jsonl(fpath)
    logger.info(f"[msmarco_mqa_conv_preserve_thinking] Loaded {len(items)} validation samples from {val_file}")
    return items
...
def _tokenize_shard(args: Tuple) -> Optional[str]:
    ...
    Since context = conversation copy, we only store conversation tokens.
    Each shard stores:
    ...
    try:
        ...
        # Save as npz (no context tokens — context is a copy of conv at collation time)
...
def _preprocess_variant(data_cfg, model_path: str, train_data: List[Dict],
    ...
    Uses preserve_thinking mode (default chat template with enable_thinking=True,
    preserve_thinking=True).
    ...
    if os.path.exists(verified_path):
        ...
        _log(f"[msmarco_mqa_conv_preserve_thinking] Removed stale VERIFIED marker")

    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa_conv_preserve_thinking] {num_samples} samples → {num_shards} shards")
    ...
    if os.path.exists(manifest_path):
        ...
        _log(f"[msmarco_mqa_conv_preserve_thinking] Manifest verified: {num_samples} samples, {num_shards} shards")
    else:
        with open(manifest_path, "w") as f:
            json.dump(manifest, f, indent=2)
        _log(f"[msmarco_mqa_conv_preserve_thinking] Manifest written: {manifest_path}")
    ...
    _log(f"[msmarco_mqa_conv_preserve_thinking] Cache status: {already_done}/{num_shards} shards done, {remaining} remaining")

    if remaining > 0:
        _log(f"[msmarco_mqa_conv_preserve_thinking] Tokenizing and caching ...")
        t0 = time.time()
        errors: List[str] = []
        ...
        if num_workers <= 1:
            ...
            for idx, item in enumerate(work_items):
                ...
                _log(f"[msmarco_mqa_conv_preserve_thinking]   {idx + 1}/{remaining} [{rate:.1f} shards/s, {elapsed:.1f}s elapsed]")
        else:
            with mp.Pool(
        ...
        elapsed = time.time() - t0
        _log(f"[msmarco_mqa_conv_preserve_thinking] Tokenization complete: {len(work_items)} shards in {elapsed:.1f}s")
        ...
        _log(f"[msmarco_mqa_conv_preserve_thinking] All shards already cached, skipping tokenization.")

    # ---- Verification ----
    _log(f"[msmarco_mqa_conv_preserve_thinking] Starting verification ...")
    _verify_cache(variant_cache_dir, num_samples, num_shards)
    _log(f"[msmarco_mqa_conv_preserve_thinking] Preprocessing complete. Cache is verified and ready.")
...
def preprocess(cfg, model_path: str):
    ...
    """
    data_cfg = cfg.data
    _raw_cache = data_cfg.get("cache_dir", "cache/msmarco_mqa_conv_preserve_thinking_tokens")
    ...
    # ---- Open .txt log file ----
    dataset_name = data_cfg.get("name", "msmarco_mqa_conv_preserve_thinking")
    ...
    _log_file.write(f"  Mode: preserve_thinking (default template, enable_thinking=True, preserve_thinking=True)\n")
    _log_file.write(f"  Note: context = conversation copy (no separate context tokenization)\n")
    _log_file.write(f"{sep}\n")
    _log_file.write(f"  Config file       : configs/data/pretrain_annealing/{dataset_name}.yaml\n")
    ...
    # ---- Load training data ----
    _log("[msmarco_mqa_conv_preserve_thinking] Loading training data ...")
    train_data = _load_train_data(data_cfg)
    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa_conv_preserve_thinking] Data loaded: {num_samples} samples → {num_shards} shards")
    ...
    _log_file.write(f"  Context           : conversation copy ✓\n")
    _log_file.write(f"  Status            : VERIFIED ✓\n")
    _log_file.write(f"{sep}\n")
    _log_file.flush()
    _log_file.close()

    logger.info(f"[msmarco_mqa_conv_preserve_thinking] Preprocess log written to: {output_path}")
...
def _verify_cache(cache_dir: str, num_samples: int, num_shards: int):
    ...
    elapsed = time.time() - t0
    logger.info(
        f"[msmarco_mqa_conv_preserve_thinking] Verification complete in {elapsed:.1f}s: "
        f"{num_samples} samples, {total_conv_tokens:,} conv tokens"
    )


# ---------------------------------------------------------------------------
# MSMarcoMQAConvPreserveThinkingDataset
# ---------------------------------------------------------------------------

class MSMarcoMQAConvPreserveThinkingDataset(BaseDataset):
    """
    Pretrain annealing dataset for MS MARCO Multi-QA (conversation-as-context,
    preserve_thinking mode).

    Loads pre-tokenized shard files and provides random access to individual
    samples. Each sample contains pre-tokenized conversation and label token ids.
    Context is a copy of conversation (handled at collation time).
    """
    ...
    def __init__(self, model_path: str, cache_dir: str):
        ...
        # Preload all shard data for fast access
        self._conv_tokens_list = []
        self._conv_lengths_list = []
        self._label_tokens_list = []

        logger.info(f"[msmarco_mqa_conv_preserve_thinking] Loading {self.num_shards} shards from {cache_dir} ...")
        t0 = time.time()
        ...
        elapsed = time.time() - t0
        logger.info(f"[msmarco_mqa_conv_preserve_thinking] Loaded {self.num_samples} samples in {elapsed:.1f}s")
...
# ---------------------------------------------------------------------------
# MSMarcoMQAConvPreserveThinkingCollator
# ---------------------------------------------------------------------------

class MSMarcoMQAConvPreserveThinkingCollator(BaseCollator):
    """
    Collator for MSMarcoMQAConvPreserveThinkingDataset.

    Context is a COPY of conversation tokens. Pads/truncates to fixed lengths.

    Output batch (unified format):
        - context_ids:      (B, conv_seq_length + num_mem_token)
        - conversation_ids: (B, conv_seq_length)
        - labels:           (B, conv_seq_length)  with -100 for padding and non-content tokens
        - context_lengths:  (B,)  actual context lengths (= actual conv lengths, capped at conv_seq_length)
    """
    ...
    def __init__(
        ...
        # context_max_length == conversation_max_length (enforced)
    ...
    def __call__(self, samples: List[Dict[str, Any]]) -> List[Dict[str, torch.Tensor]]:
        ...
        context_total_len = conv_len + num_mem  # context_seq_length == conv_seq_length
        ...
        for i, s in enumerate(samples):
            ...
            # Context is a COPY of conversation (same tokens, same length)
            context_ids[i, :actual_conv_len] = conv[:actual_conv_len]
            context_lengths[i] = actual_conv_len
...
def create_dataset_and_collator(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    """
    Create MSMarcoMQAConvPreserveThinkingDataset and MSMarcoMQAConvPreserveThinkingCollator
    from the data configuration.

    Enforces that context_seq_length == conv_seq_length (since context is a copy
    of conversation).
    ...
    Returns:
        tuple: (MSMarcoMQAConvPreserveThinkingDataset, MSMarcoMQAConvPreserveThinkingCollator)
    """
    data_cfg = cfg.data
    ...
    # Enforce: context_seq_length must equal conv_seq_length
    assert context_seq_len == conv_seq_len, (
        f"msmarco_mqa_conv_preserve_thinking requires context_seq_length == conv_seq_length, "
        f"but got context_seq_length={context_seq_len}, conv_seq_length={conv_seq_len}. "
        f"Since context is a copy of conversation, they must be identical."
    )

    cache_dir = _resolve_cache_dir(data_cfg)

    logger.info(
        f"[msmarco_mqa_conv_preserve_thinking] Creating dataset: "
        f"conv_seq_len={conv_seq_len} (= context_seq_len), "
        f"cache_dir='{cache_dir}', num_mem_token={num_mem_token}"
    )

    # Build dataset
    train_ds = MSMarcoMQAConvPreserveThinkingDataset(
        model_path=model_path,
        cache_dir=cache_dir,
    )

    # Build collator
    collator = MSMarcoMQAConvPreserveThinkingCollator(
        model_path=model_path,
...
class MSMarcoMQAConvPreserveThinkingValDataset(BaseDataset):
    """
    Validation dataset for MS MARCO Multi-QA (conversation-as-context,
    preserve_thinking mode).

    Loads val.jsonl directly and tokenizes on-the-fly (since val set is small).
    Returns the same format as MSMarcoMQAConvPreserveThinkingDataset for compatibility.
    """
...
def create_val_dataset(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    ...
    If ``cfg.data.validation_subset_num`` is set (and != -1), a reproducible
    subset of the full validation set is selected using ``cfg.seed.dataset``
    as the random seed.
    ...
    Returns:
        MSMarcoMQAConvPreserveThinkingValDataset (or a Subset of it), or None if val_file is
    ...
    if not os.path.exists(val_path):
        logger.warning(f"[msmarco_mqa_conv_preserve_thinking] Validation file not found: {val_path}")
        return None

    logger.info(f"[msmarco_mqa_conv_preserve_thinking] Creating validation dataset from {val_file}")
    full_val_ds = MSMarcoMQAConvPreserveThinkingValDataset(model_path=model_path, data_cfg=data_cfg, tokenizer_cfg=cfg.tokenizer)
    ...
    if subset_size <= 0 or subset_size >= total_val:
        logger.info(
            f"[msmarco_mqa_conv_preserve_thinking] Using full validation set: {total_val} samples "
    ...
    logger.info(
        f"[msmarco_mqa_conv_preserve_thinking] Validation subset: {subset_size}/{total_val} samples "
...
def debug(cfg, model_path: str):
    ...
    print(f"\n{sep}")
    print(f"  DEBUG — msmarco_mqa_conv_preserve_thinking (context = conversation copy)")
    print(f"{sep}\n")
    ...
    metadata = {
        "conv_seq_len": data_cfg.conv_seq_length,
        "context_seq_len": data_cfg.context_seq_length,
        "context_mode": "conversation_copy",
        "template_mode": "preserve_thinking",
        "cache_dir": cache_dir,
    ...
    debug_dataset(
        ...
        collator=collator,
        tokenizer=tokenizer,
        dataset_name=f"{data_cfg.get('name', 'msmarco_mqa_conv_preserve_thinking')}_preserve_thinking",
        metadata=metadata,
        num_samples=5,
        num_mem_token=num_mem_token,
        pad_token_id=pad_token_id,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="msmarco_mqa_conv_preserve_thinking pretrain_annealing dataset utilities")
    ...
    parser.add_argument("--config", type=str, default="configs/data/pretrain_annealing/msmarco_mqa_conv_preserve_thinking.yaml",
    ...
    if model_path is None:
        for _cfg_name in ["main_pretrain.yaml", "main_pretrain_annealing.yaml", "main_sft.yaml"]:
