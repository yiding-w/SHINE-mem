...
dataset. Unlike the SFT version where context is the evidence passage, here the
...
Preprocessing:
    Tokenizes all samples using the tokenizer's chat template
    (``apply_chat_template``) for the conversation part, saves as .npz
...
Collator:
    Pads/truncates to fixed lengths and produces the unified batch format:
        ...
        - labels:           (B, conv_seq_length)  with -100 for question tokens
...
def _load_train_data(data_cfg) -> List[Dict]:
    ...
    for fname in train_files:
        ...
        logger.info(f"[msmarco_mqa_conv] Loaded {len(items)} samples from {fname}")
        all_items.extend(items)

    logger.info(f"[msmarco_mqa_conv] Total training samples: {len(all_items)}")
    return all_items
...
def _load_val_data(data_cfg) -> List[Dict]:
    ...
    items = _load_jsonl(fpath)
    logger.info(f"[msmarco_mqa_conv] Loaded {len(items)} validation samples from {val_file}")
    return items
...
def _build_label_mask_chat(
    tokenizer,
    conv_token_ids: List[int],
) -> List[int]:
    """
    Build label mask for chat-template-encoded conversation:
    mask everything except assistant response tokens with -100.

    Strategy: scan for ``<|im_start|> assistant`` patterns to find
    assistant turn boundaries, then keep only the response content
    (between the role header newline and the ``<|im_end|>`` token).
    ...
    nl_token_id = nl_id[0] if nl_id else None
    ...
    while i < n:
        ...
        if (
            ids[i] == imstart_id
            and i + 1 < n and ids[i + 1] == assistant_id
        ):
            # Find the newline after "assistant" — content starts after it
            header_end = i + 2
            if nl_token_id is not None and header_end < n and ids[header_end] == nl_token_id:
                header_end += 1
            ...
            # Unmask assistant content tokens AND the trailing <|im_end|>
...
def _tokenize_shard(args: Tuple) -> Optional[str]:
    ...
    try:
        ...
        for sample in samples:
            ...
            # Format as chat messages and encode with chat template
            ...
            labels = _build_label_mask_chat(_worker_tokenizer, conv_ids)
...
def _preprocess_variant(data_cfg, model_path: str, train_data: List[Dict],
    ...
    if os.path.exists(verified_path):
        ...
        _log(f"[msmarco_mqa_conv] Removed stale VERIFIED marker")

    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa_conv] {num_samples} samples → {num_shards} shards")
    ...
    if os.path.exists(manifest_path):
        ...
        _log(f"[msmarco_mqa_conv] Manifest verified: {num_samples} samples, {num_shards} shards")
    else:
        with open(manifest_path, "w") as f:
            json.dump(manifest, f, indent=2)
        _log(f"[msmarco_mqa_conv] Manifest written: {manifest_path}")
    ...
    _log(f"[msmarco_mqa_conv] Cache status: {already_done}/{num_shards} shards done, {remaining} remaining")

    if remaining > 0:
        _log(f"[msmarco_mqa_conv] Tokenizing and caching ...")
        t0 = time.time()
        errors: List[str] = []
        ...
        if num_workers <= 1:
            ...
            for idx, item in enumerate(work_items):
                ...
                _log(f"[msmarco_mqa_conv]   {idx + 1}/{remaining} [{rate:.1f} shards/s, {elapsed:.1f}s elapsed]")
        else:
            with mp.Pool(
        ...
        elapsed = time.time() - t0
        _log(f"[msmarco_mqa_conv] Tokenization complete: {len(work_items)} shards in {elapsed:.1f}s")
        ...
        _log(f"[msmarco_mqa_conv] All shards already cached, skipping tokenization.")

    # ---- Verification ----
    _log(f"[msmarco_mqa_conv] Starting verification ...")
    _verify_cache(variant_cache_dir, num_samples, num_shards)
    _log(f"[msmarco_mqa_conv] Preprocessing complete. Cache is verified and ready.")
...
def preprocess(cfg, model_path: str):
    ...
    Uses the nothinking template (always).
    ...
    """
    data_cfg = cfg.data
    _raw_cache = data_cfg.get("cache_dir", "cache/msmarco_mqa_conv_tokens")
    ...
    # ---- Open .txt log file ----
    dataset_name = data_cfg.get("name", "msmarco_mqa_conv")
    ...
    _log_file.write(f"  Mode: nothinking (no-thinking template)\n")
    ...
    # ---- Load training data ----
    _log("[msmarco_mqa_conv] Loading training data ...")
    train_data = _load_train_data(data_cfg)
    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa_conv] Data loaded: {num_samples} samples → {num_shards} shards")

    # ---- Process (nothinking template only) ----
    try:
        _preprocess_variant(
            data_cfg=data_cfg,
    ...
    _log_file.write(f"  Template          : nothinking ✓\n")
    ...
    logger.info(f"[msmarco_mqa_conv] Preprocess log written to: {output_path}")
...
def _verify_cache(cache_dir: str, num_samples: int, num_shards: int):
    ...
    elapsed = time.time() - t0
    logger.info(
        f"[msmarco_mqa_conv] Verification complete in {elapsed:.1f}s: "
        f"{num_samples} samples, {total_conv_tokens:,} conv tokens"
    )


# ---------------------------------------------------------------------------
# MSMarcoMQAConvDataset
# ---------------------------------------------------------------------------

class MSMarcoMQAConvDataset(BaseDataset):
    """
    ...
    def __init__(self, model_path: str, cache_dir: str):
        ...
        logger.info(f"[msmarco_mqa_conv] Loading {self.num_shards} shards from {cache_dir} ...")
        t0 = time.time()
        ...
        elapsed = time.time() - t0
        logger.info(f"[msmarco_mqa_conv] Loaded {self.num_samples} samples in {elapsed:.1f}s")
...
# ---------------------------------------------------------------------------
# MSMarcoMQAConvCollator
# ---------------------------------------------------------------------------

class MSMarcoMQAConvCollator(BaseCollator):
    """
    Collator for MSMarcoMQAConvDataset.
    ...
    Output batch (unified format):
        ...
        - conversation_ids: (B, conv_seq_length)
        - labels:           (B, conv_seq_length)  with -100 for padding and question tokens
...
def create_dataset_and_collator(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    """
    Create MSMarcoMQAConvDataset and MSMarcoMQAConvCollator from the data configuration.
    ...
    Returns:
        tuple: (MSMarcoMQAConvDataset, MSMarcoMQAConvCollator)
    """
    data_cfg = cfg.data
    ...
    assert context_seq_len == conv_seq_len, (
        f"msmarco_mqa_conv requires context_seq_length == conv_seq_length, "
    ...
    logger.info(
        f"[msmarco_mqa_conv] Creating dataset: "
        f"conv_seq_len={conv_seq_len} (= context_seq_len), "
        f"cache_dir='{cache_dir}', num_mem_token={num_mem_token}"
    )

    # Build dataset
    train_ds = MSMarcoMQAConvDataset(
        model_path=model_path,
        cache_dir=cache_dir,
    )

    # Build collator
    collator = MSMarcoMQAConvCollator(
        model_path=model_path,
...
class MSMarcoMQAConvValDataset(BaseDataset):
    """
    ...
    Returns the same format as MSMarcoMQAConvDataset for compatibility.
    """

    def __init__(self, model_path: str, data_cfg, tokenizer_cfg=None):
        super().__init__(model_path, tokenizer_cfg=tokenizer_cfg)

        self.tokenizer = create_tokenizer(model_path, tokenizer_cfg=tokenizer_cfg, chat_template=NOTHINKING_CHAT_TEMPLATE)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Build labels: mask non-assistant tokens
        labels = _build_label_mask_chat(self.tokenizer, conv_ids)
...
def create_val_dataset(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    ...
    Returns:
        MSMarcoMQAConvValDataset (or a Subset of it), or None if val_file is
    ...
    if not os.path.exists(val_path):
        logger.warning(f"[msmarco_mqa_conv] Validation file not found: {val_path}")
        return None

    logger.info(f"[msmarco_mqa_conv] Creating validation dataset from {val_file}")
    full_val_ds = MSMarcoMQAConvValDataset(model_path=model_path, data_cfg=data_cfg, tokenizer_cfg=cfg.tokenizer)
    ...
    if subset_size <= 0 or subset_size >= total_val:
        logger.info(
            f"[msmarco_mqa_conv] Using full validation set: {total_val} samples "
    ...
    logger.info(
        f"[msmarco_mqa_conv] Validation subset: {subset_size}/{total_val} samples "
...
def debug(cfg, model_path: str):
    """
    Debug the dataset (nothinking template).
    ...
    print(f"\n{sep}")
    print(f"  DEBUG — msmarco_mqa_conv (context = conversation copy)")
    print(f"{sep}\n")
    ...
    debug_dataset(
        ...
        collator=collator,
        tokenizer=tokenizer,
        dataset_name=f"{data_cfg.get('name', 'msmarco_mqa_conv')}_nothinking",
        metadata=metadata,
        num_samples=5,
        num_mem_token=num_mem_token,
        pad_token_id=pad_token_id,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="msmarco_mqa_conv pretrain_annealing dataset utilities")
    ...
    parser.add_argument("--config", type=str, default="configs/data/pretrain_annealing/msmarco_mqa_conv.yaml",
