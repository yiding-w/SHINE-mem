...
# -*- coding: utf-8 -*-
"""
MS MARCO Multi-QA SFT Dataset Module for SHINE_V2

This module provides a supervised fine-tuning (SFT) dataset based on the
MS MARCO Multi-QA dataset. Each sample consists of a context passage and
a multi-turn QA conversation.
...
Preprocessing:
    ...
    shard files. Supports resume. After tokenization, a verification pass
    checks every shard for correctness and writes a VERIFIED marker.
    Labels mask all non-assistant tokens with -100.
...
def _load_train_data(data_cfg) -> List[Dict]:
    ...
    for fname in train_files:
        ...
        logger.info(f"[msmarco_mqa] Loaded {len(items)} samples from {fname}")
        all_items.extend(items)

    logger.info(f"[msmarco_mqa] Total training samples: {len(all_items)}")
    return all_items
...
def _load_val_data(data_cfg) -> List[Dict]:
    ...
    items = _load_jsonl(fpath)
    logger.info(f"[msmarco_mqa] Loaded {len(items)} validation samples from {val_file}")
    return items
...
def _build_label_mask_chat(
    ...
    # nl_id is typically a single token; take the first one
    ...
    while i < n:
        ...
        if (
            ...
            # (model should learn when to stop generating)
...
def _tokenize_shard(args: Tuple) -> Optional[str]:
    ...
    try:
        ...
        for sample in samples:
            ...
            # Tokenize context (plain text, no chat template)
            ctx_ids = _worker_tokenizer.encode(context, add_special_tokens=False)
...
def _preprocess_variant(data_cfg, model_path: str, train_data: List[Dict],
    ...
    Always uses the nothinking template.
    ...
    if os.path.exists(verified_path):
        ...
        _log(f"[msmarco_mqa] Removed stale VERIFIED marker")

    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa] {num_samples} samples → {num_shards} shards")
    ...
    if os.path.exists(manifest_path):
        ...
        _log(f"[msmarco_mqa] Manifest verified: {num_samples} samples, {num_shards} shards")
    else:
        with open(manifest_path, "w") as f:
            json.dump(manifest, f, indent=2)
        _log(f"[msmarco_mqa] Manifest written: {manifest_path}")
    ...
    _log(f"[msmarco_mqa] Cache status: {already_done}/{num_shards} shards done, {remaining} remaining")

    if remaining > 0:
        _log(f"[msmarco_mqa] Tokenizing and caching ...")
        t0 = time.time()
        errors: List[str] = []
        ...
        if num_workers <= 1:
            ...
            for idx, item in enumerate(work_items):
                ...
                _log(f"[msmarco_mqa]   {idx + 1}/{remaining} [{rate:.1f} shards/s, {elapsed:.1f}s elapsed]")
        else:
            with mp.Pool(
        ...
        elapsed = time.time() - t0
        _log(f"[msmarco_mqa] Tokenization complete: {len(work_items)} shards in {elapsed:.1f}s")
        ...
        _log(f"[msmarco_mqa] All shards already cached, skipping tokenization.")

    # ---- Verification ----
    _log(f"[msmarco_mqa] Starting verification ...")
    _verify_cache(variant_cache_dir, num_samples, num_shards)
    _log(f"[msmarco_mqa] Preprocessing complete. Cache is verified and ready.")
...
def preprocess(cfg, model_path: str):
    ...
    """
    data_cfg = cfg.data
    _raw_cache = data_cfg.get("cache_dir", "cache/msmarco_mqa_tokens")
    ...
    # ---- Open .txt log file ----
    dataset_name = data_cfg.get("name", "msmarco_mqa")
    ...
    # ---- Load training data ----
    _log("[msmarco_mqa] Loading training data ...")
    train_data = _load_train_data(data_cfg)
    num_samples = len(train_data)
    num_shards = (num_samples + texts_per_shard - 1) // texts_per_shard
    _log(f"[msmarco_mqa] Data loaded: {num_samples} samples → {num_shards} shards")
    ...
    logger.info(f"[msmarco_mqa] Preprocess log written to: {output_path}")
...
def _verify_cache(cache_dir: str, num_samples: int, num_shards: int):
    ...
    elapsed = time.time() - t0
    logger.info(
        f"[msmarco_mqa] Verification complete in {elapsed:.1f}s: "
...
class MSMarcoMQADataset(BaseDataset):
    """
    SFT dataset for MS MARCO Multi-QA.
    ...
    def __init__(self, model_path: str, cache_dir: str):
        ...
        logger.info(f"[msmarco_mqa] Loading {self.num_shards} shards from {cache_dir} ...")
        t0 = time.time()
        ...
        elapsed = time.time() - t0
        logger.info(f"[msmarco_mqa] Loaded {self.num_samples} samples in {elapsed:.1f}s")
...
class MSMarcoMQACollator(BaseCollator):
    """
    Collator for MSMarcoMQADataset.
...
def create_dataset_and_collator(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    """
    Create MSMarcoMQADataset and MSMarcoMQACollator from the data configuration.
    ...
    Returns:
        tuple: (MSMarcoMQADataset, MSMarcoMQACollator)
    """
    data_cfg = cfg.data
    ...
    logger.info(
        f"[msmarco_mqa] Creating dataset: context_seq_len={context_seq_len}, "
        f"conv_seq_len={conv_seq_len}, "
        f"cache_dir='{cache_dir}', num_mem_token={num_mem_token}"
    )

    # Build dataset
    train_ds = MSMarcoMQADataset(
        model_path=model_path,
        cache_dir=cache_dir,
    )

    # Build collator
    collator = MSMarcoMQACollator(
        model_path=model_path,
...
class MSMarcoMQAValDataset(BaseDataset):
    """
    Validation dataset for MS MARCO Multi-QA.

    Loads val.jsonl directly and tokenizes on-the-fly (since val set is small).
    Returns the same format as MSMarcoMQADataset for compatibility.
    """
...
def create_val_dataset(cfg, model_path: str, pad_token_id: int, num_mem_token: int = 0):
    ...
    This function is called by create_dataset_from_config in mydata.py
    when the dataset module provides its own validation set.
    ...
    as the random seed.  This guarantees the same subset is chosen every
    time regardless of the number of processes or run order.

    Supported values for ``validation_subset_num``:
        - -1:              use the entire validation set
        - int (>= 1):      use exactly that many samples
        - float (0, 1):    use that fraction of the validation set
    ...
    Returns:
        MSMarcoMQAValDataset (or a Subset of it), or None if val_file is
    ...
    if not os.path.exists(val_path):
        logger.warning(f"[msmarco_mqa] Validation file not found: {val_path}")
        return None

    logger.info(f"[msmarco_mqa] Creating validation dataset from {val_file}")
    full_val_ds = MSMarcoMQAValDataset(model_path=model_path, data_cfg=data_cfg, tokenizer_cfg=cfg.tokenizer)
    ...
    if subset_size <= 0 or subset_size >= total_val:
        ...
        logger.info(
            f"[msmarco_mqa] Using full validation set: {total_val} samples "
    ...
    logger.info(
        f"[msmarco_mqa] Validation subset: {subset_size}/{total_val} samples "
...
def debug(cfg, model_path: str):
    ...
    print(f"\n{sep}")
    print(f"  DEBUG — nothinking template")
    print(f"{sep}\n")
    ...
    debug_dataset(
        ...
        collator=collator,
        tokenizer=tokenizer,
        dataset_name=f"{data_cfg.get('name', 'msmarco_mqa')}_nothinking",
        metadata=metadata,
        num_samples=5,
        num_mem_token=num_mem_token,
        pad_token_id=pad_token_id,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="msmarco_mqa SFT dataset utilities")
    ...
    parser.add_argument("--config", type=str, default="configs/data/sft/msmarco_mqa.yaml",
