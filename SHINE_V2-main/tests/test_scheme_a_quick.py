"""Quick test for contiguous ring attention (Scheme A) only."""
import os
...
from utils.mytp.ring_attention import sp_flash_attention_contiguous
...
if rank == 0:
    print(f"Testing Scheme A (contiguous ring) with SP={world_size}")
...
if rank == 0:
    print(f"  Running sp_flash_attention_contiguous...")
...
dist.all_gather(out_a_gathered, out_a_local, group=dist.group.WORLD)
out_a_full = torch.cat(out_a_gathered, dim=1)

diff = (out_a_full - out_ref).abs().max().item()
if rank == 0:
    print(f"  Scheme A forward S={S}: max_diff={diff:.6e} {'PASS' if diff < 0.02 else 'FAIL'}")
