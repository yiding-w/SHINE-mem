...
def parse_args():
    ...
    parser.add_argument("--phonebook-path", type=str, help="Deprecated alias for --facts-path.")
...
def main():
    ...
    facts_path = resolve_path(args.phonebook_path or args.facts_path)
    output_path = resolve_path(args.output_path)

    facts = json.loads(facts_path.read_text(encoding="utf-8"))
    if len(facts) < total_facts:
        raise ValueError(f"Need at least {total_facts} facts, found {len(facts)} in {facts_path}")
