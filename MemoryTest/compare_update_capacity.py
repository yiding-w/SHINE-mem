...
def main():
    ...
    facts, facts_path = exp.read_json_rows(args.phonebook_path or args.facts_path, total_facts)
    ...
    lora_a, contexts_a = exp.generate_average_lora(
        fact_chunks,
        metanetwork,
        tokenizer,
        metalora,
    ...
    if args.save_loras:
        exp.save_lora_snapshot(output_path.with_name(output_path.stem + "_A_avg_lora.pt"), lora_a)
        exp.save_lora_snapshot(output_path.with_name(output_path.stem + "_B_single_lora.pt"), lora_b)

    result_a = exp.evaluate_lora(f"A_{args.num_updates}x{args.facts_per_update}_average_lora", facts, lora_a, metanetwork, tokenizer, runtime_args, device)
